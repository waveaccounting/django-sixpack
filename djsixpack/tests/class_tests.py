...
class ClientIdTest(TestCase):
	...
	def test_client_id_property_calls_method(self):
		tst = SixpackTest(Mock())
		tst.get_client_id = Mock()
		
		tst.client_id
		self.assertTrue(tst.get_client_id.called)
		
	def test_client_id_no_attr_set(self):
		tst = SixpackTest(Mock())
		tst.unique_attr = None
	...
	def test_client_id_uses_set_attr(self):
		tst = SixpackTest(Mock(pk=5))
		
		self.assertEqual(tst.client_id, 5)
